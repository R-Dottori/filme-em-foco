...
async def get_filmes():
    try:
        filmes = pd.read_csv('./data/filmes.csv').to_dict(orient='records')
        return filmes
...
@router.get('/top_250', response_model=List[ModeloFilme])
...
@router.get('/em_cartaz')
async def get_em_cartaz():
    try:
        filmes = pd.read_csv('./data/em_cartaz.csv', index_col=0)
        # filmes.drop(['analises'], axis=1, inplace=True)
